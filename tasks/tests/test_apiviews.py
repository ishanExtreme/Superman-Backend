...
class UserFunctionalityTest(APITestCase):
    ...
    def test_reset_password_with_correct_token(self):
        ...
        res = self.client.post("/api/password-reset/confirm/", {"token":token, "password": new_pass})
...
class TaskViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.stage.save()
        self.task1 = Task(title="Task with title 1", priority=1, user=self.user, due_date=datetime.datetime.now(), stage=self.stage)
        self.task1.save()
        self.task2 = Task(
